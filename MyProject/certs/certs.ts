...
export function getAspDotNetCertificate() {
	...
	if (!certificateName) {
		console.error(
			"Invalid certificate name. Run this script in the context of an npm/yarn script or pass --name=<> explicitly."
		);
		process.exit(-1);
	}
	...
	console.log(
		// prettier-ignore
		`Creating: ${path.join(baseFolder, `${certificateName}.pem`)}\nCreating: ${path.join(baseFolder, `${certificateName}.key`)}`
	);
	...
	if (!fs.existsSync(certFilePath) || !fs.existsSync(keyFilePath)) {
		...
		const fetchCert = child_process.spawnSync(
			"dotnet",
			[
				"dev-certs",
				"https",
